...
A *acting* resource featured doing something.

`fua` uses [`foaf:Agent`, "FOAF Vocabulary Specification 0.99"](http://xmlns.com/foaf/spec/#term_Agent).
...
"**A**nthropocentric **I**nformation" (**IA**) is a classification of [information](#information) and is generated by
humans and targeted to humans, so it is *human readable*.

It **MAY** also be read by machines, but reasoning is sometimes hard to archive and sometimes **NOT** possible.
If given *IA* is machine readable and the machine *understands* given piece of [data](#data),
representing *the* [information](#information) it makes sense in accordance with the wishes of a human [agent](#agent)
(like [Alice](#alice)).
...
A "**N**atural **L**anguage **P**olicy" (**NLP**) behaves as a policy or contract to be readable - and hopefully
understandable - for humans. Typically, such a policy can be spoken, or to outlive, written down.
...
A "**Nota**rization" (**NOTA**), an [Authority](#authority).
...
Operating Company is a subclass of [Operating Agent](#operating-agent).
Operating Company is a subclass of [Person](#person).
...
Operating Company is a subclass of [Operating Agent](#operating-agent).
Operating Company is a subclass of [Organization](#organization).
...
`fua` uses [`foaf:Person`, "FOAF Vocabulary Specification 0.99"](http://xmlns.com/foaf/spec/#term_Person).
